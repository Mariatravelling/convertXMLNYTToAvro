...
object createAvroFromXML extends App {
  ...
  val onlineTitles: String=output.get(4).toString
  ...
  def gettingTitles(titles: String): ListBuffer[Title] = {
    ...
    parsingTitles+=Title.apply(titles)
  ...
  val result=sc.makeRDD(Seq((Abstract,leadParagraph,url,titles,onlineTitles,dsk,alternateURL,onlineSection,printPageNumber,section,slug,
  columnNumber,seriesName,dayOfMonth,month,year,dayOfWeek,headline,content,sentence)))
  val out=sqlContext.createDataFrame(result).toDF("Abstract","leadParagraph","url","titles","onlineTitles","dsk","alternateURL","onlineSection",
  "printPageNumber","section","slug","columnNumber","seriesName","dayOfMonth","month","year","dayOfWeek","headline","content","sentences")
 // println(out.foreach(println))
  out.repartition(1).write.json("test1.json")
  out.repartition(1).write.avro("test1.avro")
