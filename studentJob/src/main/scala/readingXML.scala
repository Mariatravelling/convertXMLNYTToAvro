...
object readingXML extends App{
  ...
  val Abstract=output.get(0).toString;         println("Abstract:"+Abstract)
  ...
  def gettingtokens(tokens: String): util.ArrayList[Token] = {
    ...
    while (i < token.length) {
          ...
          val start_index: Integer = attributeOfTokens(2).substring(1).toInt
          val end_index: Integer = attributeOfTokens(3).substring(0, attributeOfTokens(3).length - 1).toInt
